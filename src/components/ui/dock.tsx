...
interface ComponentProps {
    className?: string
    children: ReactNode
    defaultAnimatingIndexes?: number[]
}

function Component({ className, children, defaultAnimatingIndexes = [] }: ComponentProps) {
    const [hovered, setHovered] = useState(false)
    const [width, setWidth] = useState(0)
    const dockRef = useRef<HTMLDivElement>(null)
    const isZooming = useRef(false)
    const [animatingIndexes, setAnimatingIndexes] = useState<number[]>(defaultAnimatingIndexes)
...
export function DockCardInner({ src, id, children, blurColor }: DockCardInnerProps) {
    ...
    return (
        <span className="relative flex justify-center items-center z-0 overflow-hidden w-full h-full rounded-md">
            {src ? (
                ...
                <motion.div
                    className={cn("absolute z-10 opacity-40 filter blur-md transform translate-y-2.5 scale-125 w-full h-full", blurColor)}
                />
            ) : null}
...
export function DockCard({ children, id, className, onClick, label, textColor }: DockCardProps) {
    ...
    const handleClick = () => {
        ...
        if (!dock.animatingIndexes.includes(index)) {
            dock.setAnimatingIndexes([...dock.animatingIndexes, index])
        } else {
            dock.setAnimatingIndexes(
                dock.animatingIndexes.filter((i) => i !== index)
            )
        }
    }
    ...
    useEffect(() => {
        const index = parseInt(id)
        const isSelected = dock.animatingIndexes.includes(index)
        ...
        } else if (!isSelected && isAnimating.current) {
    ...
    }, [dock.animatingIndexes, id, controls, opacity])
    ...
    // Base width (40) + approx char width (12) * length + padding (36)
    const expandedWidth = useMemo(() => {
        return 40 + (label ? label.length * 12 + 36 : 0)
    }, [label])

    // Tighter range to prevent neighbor magnification (fisheye effect)
    let widthSync = useTransform(distance, [-60, 0, 60], [40, expandedWidth, 40])
    let width = useSpring(widthSync, { mass: 0.1, stiffness: 150, damping: 12 })

    // Transform for text opacity based on width/distance
    const textOpacity = useTransform(distance, [-30, 0, 30], [0, 1, 0])
    ...
    return (
        <div className="flex flex-col items-center gap-1" key={id}>
            <motion.button
                ref={cardRef}
                className={cn(
                    "rounded-lg border dark:border-white/5 border-black/5 border-opacity-10 dark:bg-neutral-800 bg-neutral-100 saturate-90 brightness-90 transition-filter duration-200 hover:saturate-100 hover:brightness-112 flex items-center overflow-hidden",
                    className
                )}
                onClick={handleClick}
                style={{
                    width: width,
                    height: 40, // Fixed height to prevent vertical enlargement
                }}
                animate={controls}
                whileTap={{ scale: 0.95 }}
            >
                ...
                {label && (
                    <motion.span
                        ...
                        className={cn("whitespace-nowrap font-bold text-sm pr-4 pl-2", textColor || "text-neutral-700 dark:text-white")}
                    >
                        {label}
                    </motion.span>
                )}
